...
function Invoke-GitHubCi {
    ...
    try {
        ...
        if ($changes.Count -ne 0) {
            throw "Working tree is not clean. Commit and push the exact revision before starting GitHub CI."
        }
        ...
        if ($head -ne $remoteHead) {
            throw "HEAD does not match origin/$branch. Push the exact revision before starting GitHub CI."
        }
        ...
        Write-Host "Watching GitHub CI workflow run $($run.databaseId) for commit $head"
        gh run watch $run.databaseId --exit-status
        if ($LASTEXITCODE -ne 0) {
            throw "GitHub CI failed. Inspect it with: gh run view $($run.databaseId) --log-failed"
        }

        Write-Host "GitHub CI passed for commit $head"
    } finally {
        Pop-Location
    }
}
