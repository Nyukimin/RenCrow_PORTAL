...
- `mode=IdleChat`: 閲覧専用。COREへの更新要求を許可しない。旧`mode=view`は互換エイリアスとして受理する。
...
新しいAPIを中継する場合は、methodとpathをallowlistへ追加し、`IdleChat`／`live`からwriteできないテストとdebug/admin APIを遮断するテストを必須とする。
