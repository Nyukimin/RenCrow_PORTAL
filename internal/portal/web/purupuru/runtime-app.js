...
(function registerPuruPuruRuntime(hostWindow) {
  ...
  const indexedDB = runtime.indexedDB || hostWindow.indexedDB;
  ...
  const PORTAL_CHARACTER = Object.hasOwn(PORTAL_CHARACTER_DIRS, String(runtime.character || "").toLowerCase())
    ...
    : "mio";
  const PORTAL_CHARACTER_DIR = PORTAL_CHARACTER_DIRS[PORTAL_CHARACTER];
  const OBS_PRESETS = {
  ...
  const OBS_TRANSPARENT = (OBS_MODE || PORTAL_RUNTIME_MODE) && URL_PARAMS.get("transparent") !== "0";
  const OBS_INPUT_FETCH_TIMEOUT_MS = 2000;

  function portalAssetURL(path) {
  ...
  const ASSETS = PORTAL_RUNTIME_MODE ? portalAvatarAssets(PORTAL_CHARACTER_DIR) : DEFAULT_ASSETS;
  ...
  const DEFAULT_SETTINGS_URL = PORTAL_RUNTIME_MODE
  ...
  function applyObsModeDefaults() {
    ...
    if (!OBS_MODE || PORTAL_RUNTIME_MODE) return;
  ...
  function connectObsEventSource() {
    if (!OBS_MODE || PORTAL_RUNTIME_MODE || obsEventSource) return;
    try {
  ...
  function updatePointerTarget(clientX, clientY) {
    ...
    if (!panelRectCache || now - panelRectCacheAt > 250) {
      panelRectCache = PORTAL_RUNTIME_MODE && Number.isFinite(Number(runtime.controlPanelLeft))
        ? { left: clamp(Number(runtime.controlPanelLeft), 0, stage.w) }
        : (document.querySelector(".control-card")?.getBoundingClientRect() || null);
  ...
  function requestMainAnimationFrame() {
    if (PORTAL_RUNTIME_MODE || mainAnimationPaused || mainRafId !== null) return;
  ...
  function tick(timestamp) {
    try {
      const obsFrameInterval = OBS_MODE && !PORTAL_RUNTIME_MODE ? 1000 / currentObsRenderFps() : 0;
      const obsFrameSkipThreshold = Math.max(0, obsFrameInterval - 8);
      if (OBS_MODE && !PORTAL_RUNTIME_MODE && obsLastFrameAt && timestamp - obsLastFrameAt < obsFrameSkipThreshold) {
        return;
      }
      if (OBS_MODE && !PORTAL_RUNTIME_MODE) obsLastFrameAt = timestamp;
